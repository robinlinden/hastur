    ...
    constexpr void constexpr_test(std::string name, auto test) {
        static_assert([test] {
            struct ConstexprActions : IActions {
                void expect(bool b, std::optional<std::string_view>, etest::source_location const &) noexcept override {
                    if (!b) {
                        std::terminate();
                    }
                }

                void require(bool b, std::optional<std::string_view>, etest::source_location const &) override {
                    if (!b) {
                        std::terminate();
                    }
                }
