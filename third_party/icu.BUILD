...
LDICUDTFLAGS=
LD_SONAME=-Wl,-compatibility_version -Wl,78 -Wl,-current_version -Wl,78.2 -install_name
RPATH_FLAGS=
BIR_LDFLAGS=
AR=ar
ARFLAGS=r -c
...
genrule(
    ...
    cmd = r"""
        ...
        $(location pkgdata) --bldopt $(location pkgdata_inc) --entrypoint icudt78 --sourcedir $(RULEDIR) --destdir $(RULEDIR) --name icudt78l --mode static $${srcs[0]}
    """,
    tools = [
        ":icupkg",
        ":pkgdata",
...
genrule(
    ...
    cmd = r"""
        ...
        $(location pkgdata) --entrypoint icudt78 --sourcedir $(RULEDIR) --destdir $(RULEDIR) --name icudt78l --mode static $${srcs[0]}
    """,
    tools = [
        ":icupkg",
        ":pkgdata",
...
cc_import(
    ...
    static_library = select({
        "@platforms//os:windows": ":run_pkgdata_windows",
        "//conditions:default": ":run_pkgdata",
    }),
    visibility = ["//visibility:public"],
    alwayslink = True,
)
