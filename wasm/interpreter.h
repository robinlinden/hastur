...
// TODO(robinlinden): Validation will check that everything makes sense before
// we're allowed to run any wasm.
// TODO(robinlinden): This should loaded with modules for setup outside of
// testing.
class Interpreter {
public:
    ...
    std::optional<Value> run(std::span<instructions::Instruction> const &insns) {
    ...
    // t.const c
    void interpret(instructions::I32Const const &v) { stack.push_back(v.value); }
    ...
    void interpret(instructions::I32LessThanSigned const &) {
        ...
        auto rhs = stack.back();
        stack.pop_back();
        stack.push_back(std::get<std::int32_t>(lhs) < std::get<std::int32_t>(rhs) ? 1 : 0);
    }
