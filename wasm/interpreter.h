...
public:
    ...
    // TODO(robinlinden): Better error-reporting.
    std::optional<Interpreter> create(wasm::Module const &module) {
        if (auto res = validation::validate(module); !res) {
            std::cerr << "Validation failed: " << to_string(res.error()) << '\n';
            return std::nullopt;
        }

        // TODO(robinlinden) Imports.
        if (module.import_section.has_value() && !module.import_section->imports.empty()) {
            std::cerr << "Imports not supported yet\n";
            return std::nullopt;
        }
    ...
    void interpret(instructions::I32LessThanSigned const &) {
        ...
        auto rhs = stack.back();
        stack.pop_back();
        stack.emplace_back(std::get<std::int32_t>(lhs) < std::get<std::int32_t>(rhs) ? 1 : 0);
    }
