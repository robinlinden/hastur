...
struct Leb128<T> {
    static std::expected<T, Leb128ParseError> decode_from(std::istream &&is) { return decode_from(is); }
    static std::expected<T, Leb128ParseError> decode_from(std::istream &is) {
        T result{};
        std::uint8_t shift{};
        ...
        for (int i = 0; i < max_bytes; ++i) {
            ...
            if (!is.read(reinterpret_cast<char *>(&byte), sizeof(byte))) {
                return std::unexpected{Leb128ParseError::UnexpectedEof};
            }
            ...
            if (i == max_bytes - 1) {
                ...
                if (extra_bits != 0) {
                    return std::unexpected{Leb128ParseError::NonZeroExtraBits};
                }
            }
        ...
        return std::unexpected{Leb128ParseError::Invalid};
    }
};
